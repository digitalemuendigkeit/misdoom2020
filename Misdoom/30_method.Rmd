...
Potential participants who reported only passive social media use or none at all were excluded. The _moderately active_ group (group _MA_, n=5) included participants who mostly consumed social media but only seldomly posted or commented. Participants who not only used but also commented and posted in social media became part of the _very active group_ (group _VA_, n=6). 
...
edu <- c(paste('Abitur', footnote_marker_alphabet(1), ': ', eduabi[1], ', University Degree: ', eduuni[1], sep = ""), paste('Abitur', footnote_marker_alphabet(1), ': ',  eduabi[2], ', University Degree: ', eduuni[2], sep = ""))
occ <- c(paste('Student: ', sum(pf[1:5, 15]=='Student'), ', Full-Time Employed: ', (5 - sum(pf[1:5, 15]=='Student')), sep = ""), paste('Student: ', sum(pf[6:11, 15]=='Student'), ', Full-Time Employed: ', (6 - sum(pf[6:11, 15]=='Student')), sep = ""))
...
names (participants) <- c("Group", "Gender", "Age", "Highest Level of Education", "Occupation", paste0("Frequency of Social Media Use", footnote_marker_alphabet(2), footnote_marker_alphabet(3)), paste0("Frequency of Posting and Commenting Online", footnote_marker_alphabet(2)), paste0("Political Left-Right Self-Placement", footnote_marker_alphabet(4)))
participants[1]<- NULL
...
parttrans %>% 
  knitr::kable(caption = "Focus group participants", booktabs=T, row.names = T, escape = F) %>% 
  column_spec(1, width = "10em") %>% 
  column_spec(2:3, width = "10em") %>% 
  noop() %>%
  footnote(threeparttable = T, alphabet = c("General Higher Education Entrance Qualification;", "1=never, 2=very rarely, 3=several times a month, 4=several times a week, 5=daily, 6=several times a day;", "averaged over 6 types of platforms (social networking sites, video platforms, blogs, online newspapers, infotainment, social news);","1=left, 10=right"))
...
The participants from group _VA_ score somewhat higher on average social media use frequency and much higher on the posting and commenting frequency. Therefore, the classification based on the preliminary questioning was proven valid.

## Content analysis
We conducted a qualitative content analysis as described by @mayringQualitativeContentAnalysis2014 using MAXQDA. <!-- Spefically, we employed the summarizing technique. 
