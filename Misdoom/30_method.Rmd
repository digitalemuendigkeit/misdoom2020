...
Data was gathered in two focus groups. Afterwards, the data was transcribed and analysed to find the most pertinent predictors. The full transcriptions as well as the full analysis can be found in our github repository for this project.[^github]
...
Each focus group was recorded on audio. The recordings were then transcribed using MAXQDA. The transcription system employed was a modified version of GAT 2 [@seltingSystemTranscribingTalkininteraction2011]. The full transcripts can be found in the accompanying github repository.

## Participants
The participants were recruited through convenience sampling. Based on preliminary questioning, they were sorted into two homogenous groups, the moderately active group and the very active group, in order to obtain more detailed results [@schulzFokusgruppenEmpirischenSozialwissenschaft2012]. 
Potential participants who reported only passive social media use or none at all were excluded. Participants who mostly consumed social media but only seldom posted or commented were sorted into the moderately active group (n=5), while participants who both used and commented and posted in social media became part of the very active group (n=6).
Before starting the focus group, each participant was surveyed on some demographic details as well as the frequency of their social media use, the frequency of them posting and commenting online, and their political left-right self-placement [@breyerLeftRightSelfPlacementALLBUS]. A sample of the questionnaire can be found in the github repository. The results of that survey are displayed in Table \@ref(tab:table-participants). 
...
# Setup data for this table
group <-
  c('Moderately active group (n=5)', 'Very active group (n=6)')


table(pf$Highest.Level.of.Education)
my_t <- pf %>% 
  group_by(Group = Group.Number) %>% 
  summarise(males = table(Gender)["m"],
            females = table(Gender)["w"],
            Mage = round(mean(Age), digits = 1),
            SDage = round(sd(Age), digits = 1),
            EducEntry = table(Highest.Level.of.Education)[1],
            EducUniversity = table(Highest.Level.of.Education)[2]) %>% 
...
```{r make-participants2, include=FALSE}
...
parttrans %>% 
  knitr::kable(caption = "Focus group participants", booktabs=T, row.names = T) %>% 
...
In Figure \@ref(fig:categorization), an overview of the final categorization is visible. The full categorization can be found somewhere.
